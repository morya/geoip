...
//go:embed qqzeng-ip-china-utf8.dat
var ipdata []byte
...
func loadIpDat() (*ipSearch, error) {
	...
	p.firstStartIpOffset = bytesToLong(p.data[0], p.data[1], p.data[2], p.data[3])
	p.prefixStartOffset = bytesToLong(p.data[8], p.data[9], p.data[10], p.data[11])
	p.prefixEndOffset = bytesToLong(p.data[12], p.data[13], p.data[14], p.data[15])
